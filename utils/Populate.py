...
class OpensearchManager:
    ...
    def create_index_if_not_exists(self, index_name):
        if not self._client.indices.exists(index=index_name):
            ...
            print(
                f"Index '{index_name}' created with dimension {self._embedding_dimension}."
            )
        else:
        ...
        for doc in documents:
            try:
                if not doc.page_content.strip():
                    print(
                        f"[SKIP] Empty content in document: {doc.metadata.get('source')}"
                    )
                    continue
                ...
                if embedding_vector is None:
                    print(
                        f"[SKIP] Embedding returned None for document: {doc.metadata.get('source')}"
                    )
                    continue

                if not isinstance(embedding_vector, list) or not all(
                    isinstance(x, (float, int)) for x in embedding_vector
                ):
                    print(
                        f"[SKIP] Invalid embedding format for document: {doc.metadata.get('source')}"
                    )
                    continue

                if len(embedding_vector) != self._embedding_dimension:
                    print(
                        f"[SKIP] Embedding dimension mismatch ({len(embedding_vector)}) for document: {doc.metadata.get('source')}"
                    )
                    continue

                doc_id = str(uuid.uuid4())
                doc_body = {"text": doc.page_content, "embedding": embedding_vector}
                self._client.index(index=self._index_name, id=doc_id, body=doc_body)
                print(
                    f"[OK] Indexed document: {doc.metadata.get('source')} with ID: {doc_id}"
                )
